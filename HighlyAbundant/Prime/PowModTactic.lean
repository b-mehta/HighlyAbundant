...
A proof-producing tactic closing `powMod a b n = m` / `≠ m` for literals. The value is computed at
elaboration time by `powModTR'` (an ordinary definition in `HighlyAbundant.Prime.PowMod`, brought
into the meta phase by `meta import`) and the kernel verifies it via `powMod_eq_of_powModTR`.
-/
